...
class GeneralSiteSettingForm extends AbstractForm
{
  ...
  protected funciton initialize():void
  {
      ...
      // add field
      $basic->addField(
          Text::create('general.site_name') // Create Text field with name "general.site_name"
      ...
      // add field
      $basic->addField(
          Text::create('general.site_title')  // Create Text field with name "general.site_title"
      ...
      $this->addFooter()
            ->addField(
                Submit::create()  // Add "submit", field use default name="_submit"
...
```php
<?php
Text::create('general.site_title')
   ->yup( // add validation
      Yup::string()
        ->required(__p('core::validation.site_title_could_not_be_blank')) // set required
...
```php
<?php
Text::create('event.enable_close')
   ->yup( // add validation
      Yup::boolean()
        ->required(__p('core::validation.this_is_required_field')) // set required
   )
...
```php
<?php
QuestionsField::create('questions')
   ->yup( // add validation
      Yup::array()
