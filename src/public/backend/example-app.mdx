...
> In this part, you will learn how to create new *Notes* package,
...
To generate an example code to a *Resource*, access **AdminCP > Installed > Apps** or use the direct URL */admin/apps*.

### Type

If *Model* is *Content*, you need to implement:

- interface: **MetaFox\Platform\Contracts\Content**
- trait: **MetaFox\Platform\Support\HasContent**.

If *Model* is *User*,  you need to implement:
...
If *Model* has privacy and MUST has custom privacy (if not, skip this step),  you need to implement:
...
This will give *model* two methods, **setPrivacyListAttribute** and **getPrivacyListAttribute**, to set/get privacy list. For example:
...
If *Model* is *Content*, and *Model* can create Feed, you will need to implement interface `MetaFox\Platform\Contracts\ActivityFeedSource`
...
If *Model* can create *Resource Stream* (using for browsing), you can implement the interface `MetaFox\Platform\Contracts\HasResourceStream`

Firstly, we will define *Blog* `PrivacyStream` class as below:
...
Note if model is *User* or *Content* Type, you must register with *global* observer. Depending on abilities of Interface, Model will listen differently with platform process.
...
To make a *Content* Model have ability to create Feed, you can define *activity type* as below:
...
In MetaFox, you can add default settings quickly when adding a *resource* to Activity Feed Form.
...
In MetaFox, we assign permissions to role in *tree*. For example, if you define a certain permission with a `UserRole::LEVEL_REGISTERED` role, then users in higher roles will have such permission also.
...
- `getUserPrivacyResource`: define all settings (defined in `getUserPrivacy` methods) used by resources. For example: you create a setting *Blog-A*, then you want to apply it to page, group v.v... you have to define them in this method only. And, you can set its phrases too.
- `getDefaultPrivacy`: define default privacy of a *resource*.
...
*Repository* can be extended from the abstract class `MetaFox\Platform\Repositories\AbstractRepository` if needed. But a *repository* is always required to implement the `Repository` interface.

For example, in the *Blog* app:
...
```
For detail, please read about *Laravel Repository* pattern

## Translation

Each app will have a folder `company/blog/resources/lang` for its language pack. For example, in the *Blog* app:
...
Language phrases can be added or modified in the respective `phrase.php` file. For example, to add/modify English phrases, we will update the  `company/blog/resources/lang/en/phrase.php` file
...
After changing language phrases in `phrase.php` file, you can run the following `php artisan` command to update:
...
### Usage
To apply translation for phrases, call `_p` method with phrase variable. Phrase variable is the combination of *app name*, *file name* and *phrase key (+ n key level)*. Use dots (.) as separators. Here are some examples to print above *Blog* phrases
...
Go to `company/blog/src/Http/Controllers/Api`, you will see some *Controller* classes. They are extended from `GatewayController` classes for multiple API versions.
...
If you need to create a *Repository* instance, you have to create by interface as below
...
```
For more details, please read *Dependence Injection*.

## Scope

If you want to query *Blog* by custom query, please do not write any custom query inside `Blog` Model class.
...
Supported *Sort* options: `recent`, `most_viewed`, `most_liked`, `most_discussed`
...
- If you want to override a certain platform Scope, for example, customize the `SortScope` in the *Photo* app, you can add the `SortScope` class as below
...
Here is the sample code to use the customized `SortScope` in *Photo* app
```php
...
- `core_privacy`:
  ...
  - *Everyone*: `privacy_id` is **1**. It's generic privacy. You can use it directly as a constant.
  - *Friends*: each User will have a privacy with `privacy_type = user_friends` and `privacy = 1`
  - *Only Me*: each User will have a privacy with `privacy_type = user_private` and `privacy = 3`
  - *Custom Privacy*: an User can have unlimited custom privacy. Example: if you created a *friend list* `id = 1`, you will have a *Privacy* record with: `item_id = 1, item_type = friend_list, privacy = 4, privacy_type = user_friend_list`.

- `core_privacy_members`: `user_id`, `privacy_id`

  - User B adds a certain item with *Friends* privacy. To verify if a user A (having `user_id` = 1 for example) can see that item or not:
...
- `core_privacy_streams`: `id`, `privacy_id`, `item_id`, `item_type`
  ...
  - If an item has *X* privacy custom friend list, it will have *X* stream records.
...
*Policy* is similar to *Access Control* in **phpFox V4**. Going to `company/blog/src/Policies`, you will see some *Policy* classes, such as `BlogPolicy` class

For example: to check if a user can create Blog, we will combine *Permission*, *Privacy*, *User Privacy*. 
...
Separate *Content* Model with Content Model's *Text*. For example: `Blog` and `BlogText` (blog_id, text, text_parsed).
