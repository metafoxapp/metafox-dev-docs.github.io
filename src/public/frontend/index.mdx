...
The MetaFox **Frontend** is developed with ReactJS. Thus, in this documentation, we assume that you are already familiar with Frontend Development with ReactJS. If you aren't still confident with Frontend Development with ReactJS, you may want to take a look at the following docs before continuing:
...
Workbox contains all libraries, tools and extensions of MetaFox Frontend. We use `lerna` to organize many packages into a single repository. Let's look overall to the top level of directory strucrure.
...
**app/dist/**: It contains bundled code for deployment.

**app/public/**: It contains static resources to build React app.

**app/src/**: It contains source code of `frontend` app.
...
There are many IDE tools supporting React Development and you can choose one IDE that you are familiar with. If you haven't got a idea about IDE for developing React, one of the most popular and powerful IDE tools that we would like to recommend to you is [VSCode](https://code.visualstudio.com/) since it has well-support for React Development with many good extensions
