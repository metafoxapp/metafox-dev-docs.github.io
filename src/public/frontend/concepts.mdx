...
Since MetaFox comes with Modular structure, Build tool help solve issues of dependencies without using `import` directly. Build tool will scan all packages declared in `settings.json`, finding all component, library to build bundle.
...
Declaration of React component is included in a listing/grid component when rendering.

In some specific cases, platform will base on the naming convention to find the best match for rendering.
...
Declaration of React component is used as embed content of other component.
