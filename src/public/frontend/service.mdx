...
In MetaFox, platform has a Service Manager called `globalContext` to manage all services. You can not access the global `manager` directly but via hooks or saga context instead.

A service after passed to global service manager can be accessible in React components or Saga functions.
...
In order to inject a React Component across apps without declaration for dependencies, the best way is to inject components into the global Service Manager by using `inject` method
...
Now, you can get CommentList from other component. For example:
