...
_MetaFox Backend_ utilizes [Laravel Service Providers](https://laravel.com/docs/9.x/packages#service-providers) to provide a robust, re-useable, and flexible way to extend platform features.
...
The app name MUST be _lowercase_ and consist of words separated by `-`, `.` or `_`. The complete name should match the regular expression `^[a-z0-9]([_.-]?[a-z0-9]+)_/[a-z0-9](<([_.]?|-{0,2})[a-z0-9]+>)\_\$`.
...
`extra.metafox.internalAdminUrl` is **required**. Used to indicate where to navigate when administrator click on the app in the AdminCP area.
