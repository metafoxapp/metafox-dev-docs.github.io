...
Let's choose the *New App* option as we intend to generate code for new app at this time.
...
Then, fill your *App name* with `metafox/note`, *schemas* with `notes`, *Model* with `Note` and *Entity Name* with `note`.
...
After adding a new API, edit `backend/packages/metafox/note/src/Http/Resources/v1/WebSetting.php` file as below

```php
<?php

namespace MetaFox\Note\Http\Resources\v1;
